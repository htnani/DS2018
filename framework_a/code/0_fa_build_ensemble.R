...
LogFitModel <- function(trait, fold, learner, fit.model){
  model.path <- paste0("../logs/fit_models/", trait, "/", learner, "_", fold, ".rds")
...
LogMetaFeatures <- function(meta.features){
  ...
  base.validation.path <- paste0("../logs/meta_features/", trait, "/validation_")
  base.test.path <- paste0("../logs/meta_features/", trait, "/test_")
  ...
  if (fold == 1){ 
    # Creates appropriate headers for the different for the metric files. 
...
FitBLUP <- function(df){
  ...
  trait.response <- mixed.solve(df$y.train, 
                                ...
                                SE = FALSE, 
                                return.Hinv = FALSE)
  LogFitModel(df$trait, df$fold, "rrBLUP", trait.response)
...
FitRF <- function(df, parallel_, ncores = NULL){
  ...
  if (parallel_ == TRUE){
    set.seed(345)
    rf <- foreach(ntree = rep(floor(ntrees/10), 10), .combine = combine, 
                  .multicombine = TRUE, .packages = 'randomForest') %dorng% {
                  randomForest(x = df$x.train, y = df$y.train, ntree = ntree)
          }   
    LogFitModel(df$trait, df$fold, "RandomForest", rf)
    ...
    rf.fit <- randomForest(x = df$x.train, 
                           y = df$y.train,
                           ntree = ntrees)
    LogFitModel(df$trait, df$fold, "RandomForest", rf.fit)
...
FitGBM <- function(df){
  ...
  gbmfit <- gbm.fit(x = df$x.train, 
                    ...
                    shrinkage = 0.1,
                    verbose = FALSE)
  LogFitModel(df$trait, df$fold, "GBM", gbmfit)
...
FitSVM <- function(df, parallel_){
  ...
  svmfit <- train(x = df$x.train, 
                  ...
                  tuneGrid = svmgrid,
                  trControl = ctrl)
  LogFitModel(df$trait, df$fold, "tSVM", svmfit)
...
FitKNNReg <- function(df, parallel_){
  ...
  knnregfit <- train(x = df$x.train, 
                     ...
                     trControl = ctrl,
                     tuneGrid = knngrid)
  LogFitModel(df$trait, df$fold, "tKNN", knnregfit)
...
FitXGBoost <- function(df){
  ...
  xgbfit <- train(x = df$x.train,
                  ...
                  trControl = ctrl,
                  tuneGrid = xgbgrid)
  LogFitModel(df$trait, df$fold, "XGB", xgbfit)
...
FitMultipleModels <- function(df, parallel_){
  ...
  meta.features <- list(validation = list(meta.features = NULL, rsquareds = NULL, 
                                          mses = NULL, rmses = NULL),
                        test = list(meta.features = NULL, rsquareds = NULL, 
                                    mses = NULL, rmses = NULL)
                       )
